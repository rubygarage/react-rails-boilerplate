...
describe('resetPassword', () => {
  ...
  describe('sendRestorePasswordEmail()', () => {
    it('success', () => {
      ...
      expect(saga.next().value).toEqual(
        call(apiClient.post, '/api/v1/auth/users/password', params.values)
      )
    ...
    it('failure', () => {
      ...
      expect(saga.throw(error).value).toEqual(
        put({ type: 'SEND_RESTORE_PASSWORD_EMAIL_SUCCESS', error })
      )
  ...
  describe('validateToken()', () => {
    ...
    it('success', () => {
      ...
      expect(saga.next().value).toEqual(
        call(apiClient.get, `/api/v1/auth/users/password?reset_token=${token}`)
      )
    })
  ...
  describe('updatePassword()', () => {
    it('success', () => {
      ...
      expect(saga.next().value).toEqual(
        put({ type: 'UPDATE_PASSWORD_SUCCESS' })
      )
    ...
    it('failure', () => {
      ...
      expect(saga.throw(error).value).toEqual(
        put({ type: 'UPDATE_PASSWORD_ERROR', error })
      )
  ...
  it('watchSendRestorePassword', () => {
    ...
    expect(watcher.next().value).toEqual(
      takeEvery('SEND_RESTORE_PASSWORD_EMAIL_REQUEST', sendRestorePasswordEmail)
    )
