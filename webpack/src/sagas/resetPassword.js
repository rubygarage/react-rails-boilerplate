...
export function* sendRestorePasswordEmail({ values, resolve, reject }) {
  ...
  try {
    yield call(apiClient.post, '/api/v1/users/password', values);
...
export function* validateToken(token, res) {
  ...
  try {
    yield call(apiClient.get, `/api/v1/users/password?reset_token=${token}`);
  } catch (error) {
...
export function* updatePassword({ values, resolve, reject }) {
  ...
  try {
    yield call(apiClient.put, '/api/v1/users/password', values);
